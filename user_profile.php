...
<div class="container" id="jumper_contain">
  ...
  <h4 class="text-center"> <b> Edit Profile </b> </h4>
...
{ 
    ...
    // stage 2 ..if no errors run query
      else if(empty($_SESSION['first_name_msg']) && empty($_SESSION['last_name_msg']) && empty($_SESSION['user_name_msg']) && empty($_SESSION['email_msg']) && empty($_SESSION['password_msg']) )
      {
          ...
          email = ?, join_date = ?, last_login = ?, user_pic = ? WHERE user_id = ? ";  
          ...
          if(!mysqli_stmt_bind_param($update_stmt,"sssssssss",$first_name,$last_name,$user_name,$password,$email,
          ...
          // update sessions if details have been changed
