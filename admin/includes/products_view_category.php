    ...
    <table class="table table-bordered table-hover table-striped " >
        ...
        <tbody>
            <?php 
            $query = "SELECT * FROM products LEFT JOIN categories ON cat_id = category WHERE category = $stock_page LIMIT $page1,10  "; 
            ...
            { ?> 
                <tr>
                    ...
                    <td> 
                        <a class="delete_btn_to_open_modal" rel="<?= $row['id']?>" id="<?= $row['title'] ?>" >
    ...
    <div id="pagination_div"> 
        ...
        if($all_posts !== 0)
        {
            ...
            <ul class="pager nav" id="pagination_list">
                    ...
                    <li class="previous">
                        <a href="products.php?product=<?=$stock_page ?>&page=<?php echo $previous ?>" class='previous'>&larr; Previous </a>
                    </li> 

                    <?php
                    $query1 = "SELECT * FROM products WHERE category = $stock_page  ";
                    ...
                    for($i = 1; $i <= $count; $i++ )
                    {
                        if($page == $i)
                        {
                            echo " <li> <a class='page_number active' href='products.php?product=$stock_page&page=$i'> $i </a> </li> ";
                        }
                        else
                        {
                            echo " <li  > <a class='page_number'  href='products.php?product=$stock_page&page=$i'> $i </a> </li> ";
                        }
                    }
                    ...
                    <li class="next">
                        <a href="products.php?product=<?=$stock_page ?>&page=<?=$next ?>" class="next">Next &rarr;</a>
                    </li>
